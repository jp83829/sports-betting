...
count <- c('R_SIG_STR.', 'B_SIG_STR.', 'R_TOTAL_STR.', 'B_TOTAL_STR.', 'R_TD', 'B_TD') #character "x of y" to be converted to percentage
...
fight <- fight_raw %>% 
  ...
  mutate_at(vars(time), ~ as.numeric(hms(.), units="mins") )
...
# FAMD
exc <- c('R_SIG_STR_pct', 'B_SIG_STR_pct', 'R_TD_pct', 'B_TD_pct', 'R_REV', 'B_REV') # supplementary variable, which is not included in the analysis
res.famd <- part_df %>%
  FAMD(ncp = 10, 
       sup.var = var(exc),  
       graph = TRUE)
